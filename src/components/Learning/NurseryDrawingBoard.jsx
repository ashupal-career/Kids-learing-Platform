...
const NurseryDrawingBoard = ({ targetLetter, onLetterComplete }) => {
  ...
  useEffect(() => {
    ...
    // Draw suggestion after context is ready
  ...
  const drawSuggestion = (context, letter) => {
    ...
    context.fillText(letter, 300, 200);
    context.restore();
  };
  ...
  const draw = (e) => {
    if (!isDrawing || completed || !ctx) return;
    const pos = getCoordinates(e);
      ...
      <div className="flex justify-between items-center mt-6">
        <div className="text-left">
          <p className="text-gray-600 text-sm">
            ✏️ Trace the letter completely to hear "Good job!"
          </p>
        </div>
        <button
          onClick={clearCanvas}
...
export default NurseryDrawingBoard;